...
/*   Updated: 2020/02/10 15:44:03 by mravily          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parsing_cub(t_appli *appli)
{
	...
	appli->player->dir = create_vector(-1, 0);
...
// 	//mlx_hook(appli.vars->win_ptr, 2, 0, handle_key_pressed, &appli);
//
...
t_vector convert_angle_to_vector(float angle)
{
	t_vector result;

	result = create_vector(
		cos(degree_to_radiant(angle)),
		sin(degree_to_radiant(angle))
	);

	return (result);
}

e_bool is_out_of_map(t_config *config, t_vector position)
{

}

t_vector cast_ray(t_config *config, t_vector position, float angle)
{
	t_vector actual;
	t_vector delta;

	delta = convert_angle_to_vector(angle);
	actual = position;
	while (is_out_of_map(config, actual) == true)
	{
